...
目前完整測試合計 118 個。測試包含離線 Electron fixtures，會驗證 production sidebar preload、overlay policy、活動窗格上方中央提示與排程 request policy，但不連線 `chatgpt.com`，也不使用使用者的登入 Session、Cookie、對話、Project 或排程資料。`npm run verify` 會執行完整 Repository 檢查與全部 118 個自動測試。
