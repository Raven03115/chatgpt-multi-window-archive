...
function loadConfig() {
  try {
    ...
    console.error(
      "[Integration v4.5.3] config load failed:",
      error.message
    );
...
function saveConfigNow() {
  try {
    ...
    console.error(
      "[Integration v4.5.3] config save failed:",
      error.message
    );
  }
}
...
async function installPaneUi(view) {
  ...
  try {
    ...
    console.error(
      "[Integration v4.5.3] pane UI injection failed:",
      error.message
    );
  }
}
      ...
      .then(async () => {
        ...
        if (isUsableWindow(workspaceWindow)) {
          workspaceWindow.setTitle(
            `ChatGPT Multi Pane v4.5.3 Beta — Active ${targetIndex + 1}/${appConfig.paneCount}`
          );
        }
      });
...
function setActivePane(index) {
  ...
  console.log(
    `[Integration v4.5.3] active pane=${activePaneIndex + 1}`
  );
...
function loadUrlInActivePane(url) {
  ...
  console.log(
    `[Integration v4.5.3] load pane=${paneIndex + 1} url=${url}`
  );
  ...
  activeView.webContents
    ...
    .catch((error) => {
      console.error(
        "[Integration v4.5.3] pane navigation failed:",
        error.message
      );
    })
    .finally(() => {
...
function completeOverlayWorkspaceSelection(url) {
  ...
  console.log(
    "[Integration v4.5.3] completing workspace selection:",
    url
  );
...
function handleSidebarNavigation(url) {
  ...
  if (shouldSuppressSidebarRouteForwarding()) {
    console.log(
      "[Integration v4.5.3] suppressed sidebar route:",
      url
    );
  }
}
...
function dismissSidebarTransientUi() {
  ...
  try {
    ...
    console.error(
      "[Integration v4.5.3] dismiss input failed:",
      error.message
    );
  }
  ...
  view.webContents.setWindowOpenHandler(({ url }) => {
    if (isChatGPTUrl(url)) {
      ...
      shell.openExternal(url).catch((error) => {
        console.error(
          "[Integration v4.5.3] external link failed:",
          error.message
        );
      });
    }
...
function schedulePaneInitialLoad(
  ...
  const load = () => {
    ...
    view.webContents
      ...
      .catch((error) => {
        console.error(
          "[Integration v4.5.3] initial pane load failed:",
          error.message
        );
      });
  };
...
function setPaneCount(targetCount) {
  ...
  try {
    ...
    console.log(
      `[Integration v4.5.3] pane count=${nextCount}`
    );
  } finally {
    setTimeout(() => {
...
function sanitizeDialogRect(
  ...
  if (
    ...
    console.log(
      "[Integration v4.5.3] rejected oversized dialog rect:",
      {
        sanitized,
        widthRatio,
        heightRatio,
...
function applyOverlayShape() {
  ...
  if (
    ...
    console.error(
      "[Integration v4.5.3] BrowserWindow.setShape unavailable"
    );
  ...
  try {
    ...
    console.error(
      "[Integration v4.5.3] setShape failed:",
      error.message
    );
  }
}
...
function unlockDialogShape() {
  ...
  console.log(
    "[Integration v4.5.3] dialog shape unlocked"
  );
}
...
function createSidebarOverlayWindow() {
  ...
  sidebarOverlayWindow.webContents.on(
    ...
    async () => {
      ...
      try {
        ...
        console.error(
          "[Integration v4.5.3] transparency CSS failed:",
          error.message
        );
      }
      ...
      console.log(
        "[Integration v4.5.3] ChatGPT sidebar overlay loaded"
      );
    }
  );
  ...
  sidebarOverlayWindow.webContents.setWindowOpenHandler(
    ({ url }) => {
      if (isExternalAccountRouteUrl(url)) {
        ...
        shell.openExternal(url).catch((error) => {
          console.error(
            "[Integration v4.5.3] sidebar external link failed:",
            error.message
          );
        });
      }
  ...
  sidebarOverlayWindow.webContents.on(
    ...
    (_event, details) => {
      console.error(
        "[Integration v4.5.3] sidebar renderer stopped:",
        details
      );
    }
  );
...
function createWorkspaceWindow() {
  workspaceWindow = new BrowserWindow({
    ...
    show: false,
    title:
      `ChatGPT Multi Pane v4.5.3 Beta — Active 1/${appConfig.paneCount}`,
...
function registerShortcut(
  ...
  try {
    ...
    console.log(
      `[Integration v4.5.3] shortcut ${label}: ` +
      `${accelerator}, registered=${registered}`
    );
  } catch (error) {
    console.error(
      `[Integration v4.5.3] shortcut ${label} failed:`,
      error.message
    );
  }
}
...
ipcMain.on(
  ...
  (event, state) => {
    ...
    if (
      ...
      console.log(
        "[Integration v4.5.3] dialog shape locked:",
        lockedDialogRect
      );
    }
...
ipcMain.on(
  ...
  (event, url) => {
    ...
    if (shouldSuppressSidebarRouteForwarding()) {
      console.log(
        "[Integration v4.5.3] suppressed sidebar route intent:",
        url
      );
    }
  }
...
app.whenReady().then(() => {
  ...
  console.log(
    "[Integration v4.5.3] Electron:",
    process.versions.electron
  );

  console.log(
    "[Integration v4.5.3] userData:",
    app.getPath("userData")
  );

  console.log(
    "[Integration v4.5.3] restored pane count:",
    appConfig.paneCount
  );
