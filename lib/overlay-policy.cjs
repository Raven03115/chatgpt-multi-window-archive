...
function buildOverlayShape(options = {}) {
  const {
    ...
    dialogRect = null,
    popupRects = [],
    captureWorkspaceInput = false
  } = options;

  if (mode === "fullscreen" || captureWorkspaceInput) {
    return [{
      x: 0,
      y: 0,
      width: bounds.width,
...
function normalizeOverlayState(state = {}) {
  ...
  return {
    ...
    generation: Number.isInteger(state.generation)
      ? state.generation
      : 0,
    overlayOnlyModal: state.overlayOnlyModal === true,
    suppressPanes:
...
function transitionOverlayState(state, event = {}) {
  ...
  let overlayOnlyModal = current.overlayOnlyModal;

  switch (event.type) {
    case "overlay-intent":
      mode = "overlay-intent-pending";
      generation += 1;
      overlayOnlyModal = true;
      break;
