...
function getAutomationsWebContentsKind(webContentsId) {
  if (
    isUsableWindow(sidebarOverlayWindow) &&
    sidebarOverlayWindow.webContents.id === webContentsId
  ) {
    return "sidebar";
  }
  ...
  if (
    paneViews.some(
      (view) =>
        isUsableView(view) &&
        view.webContents.id === webContentsId
    )
  ) {
    return "pane";
  }
...
app.whenReady().then(() => {
  ...
  configureAutomationsRequestUserAgent(
    ...
    {
      diagnosticsEnabled:
        process.env.CHATGPT_AUTOMATIONS_DIAGNOSTICS === "1",
      resolveWebContentsKind: getAutomationsWebContentsKind,
      onEvent: recordIntegrationEvent
    }
  );
