...
function applyOverlayShape() {
  ...
  if (
    ...
    console.error(
      "[PoC v3.1] BrowserWindow.setShape() is unavailable."
    );
...
function createSidebarOverlayWindow() {
  ...
  sidebarOverlayWindow.webContents.on(
    ...
    async () => {
      ...
      try {
        ...
        console.error(
          "[PoC v3.1] transparency CSS failed:",
          error.message
        );
      }
      ...
      console.log(
        "[PoC v3.1] ChatGPT overlay loaded"
      );
    }
  );
  ...
  sidebarOverlayWindow.webContents.on(
    ...
    (_event, details) => {
      console.error(
        "[PoC v3.1] sidebar renderer stopped:",
        details
      );
    }
  );
...
function registerShortcut(
  ...
  try {
    ...
    console.log(
      `[PoC v3.1] shortcut ${label}: ` +
      `${accelerator}, registered=${registered}`
    );
  } catch (error) {
    console.error(
      `[PoC v3.1] shortcut ${label} failed:`,
      error.message
    );
  }
}
