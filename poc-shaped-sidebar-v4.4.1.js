...
function getPaneBounds(index) {
  const content = getWorkspaceContentSize();
...
async function installPaneUi(view) {
  ...
  try {
    ...
    console.error(
      "[Integration v4.4.1] pane UI injection failed:",
      error.message
    );
  }
}
...
async function refreshActivePaneVisuals() {
  ...
  if (isUsableWindow(workspaceWindow)) {
    workspaceWindow.setTitle(
      `ChatGPT Multi Pane Integration — Active ${targetIndex + 1}/2`
    );
  }
}
...
function setActivePane(index) {
  const nextIndex = Math.max(
    0,
    Math.min(index, paneViews.length - 1)
  );

  activePaneIndex = nextIndex;

  console.log(
    `[Integration v4.4.1] active pane=${activePaneIndex + 1}`
  );
...
function loadUrlInActivePane(url) {
  ...
  console.log(
    `[Integration v4.4.1] load pane=${activePaneIndex + 1} url=${url}`
  );
...
function handleSidebarNavigation(url) {
  ...
  if (shouldSuppressSidebarRouteForwarding()) {
    console.log(
      "[Integration v4.4.1] suppressed sidebar route:",
      url
    );
...
function dismissSidebarTransientUi() {
  ...
  try {
    ...
    console.error(
      "[Integration v4.4.1] dismiss input failed:",
      error.message
    );
  }
  ...
  view.webContents.setWindowOpenHandler(({ url }) => {
    if (isChatGPTUrl(url)) {
      ...
      shell.openExternal(url).catch((error) => {
        console.error(
          "[Integration v4.4.1] external link failed:",
          error.message
        );
      });
    }
...
function sanitizeDialogRect(
  ...
  if (
    ...
    console.log(
      "[Integration v4.4.1] rejected oversized dialog rect:",
      {
        sanitized,
        widthRatio,
        heightRatio,
...
function applyOverlayShape() {
  ...
  if (
    ...
    console.error(
      "[Integration v4.4.1] BrowserWindow.setShape unavailable"
    );
  ...
  try {
    ...
    console.error(
      "[Integration v4.4.1] setShape failed:",
      error.message
    );
  }
}
...
function unlockDialogShape() {
  ...
  console.log(
    "[Integration v4.4.1] dialog shape unlocked"
  );
}
...
function createSidebarOverlayWindow() {
  ...
  sidebarOverlayWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      preload: path.join(
        __dirname,
        "sidebar-shape-preload-v4.4.1.js"
      ),
  ...
  sidebarOverlayWindow.webContents.on(
    ...
    async () => {
      ...
      try {
        ...
        console.error(
          "[Integration v4.4.1] transparency CSS failed:",
          error.message
        );
      }
      ...
      console.log(
        "[Integration v4.4.1] ChatGPT sidebar overlay loaded"
      );
    }
  );
  ...
  sidebarOverlayWindow.webContents.setWindowOpenHandler(
    ({ url }) => {
      if (isExternalAccountRouteUrl(url)) {
        ...
        shell.openExternal(url).catch((error) => {
          console.error(
            "[Integration v4.4.1] sidebar external link failed:",
            error.message
          );
        });
      }
  ...
  sidebarOverlayWindow.webContents.on(
    ...
    (_event, details) => {
      console.error(
        "[Integration v4.4.1] sidebar renderer stopped:",
        details
      );
    }
  );
...
function createWorkspaceWindow() {
  workspaceWindow = new BrowserWindow({
    ...
    show: false,
    title:
      "ChatGPT Multi Pane Integration v4.4.1 — Active 1/2",
...
function registerShortcut(
  ...
  try {
    ...
    console.log(
      `[Integration v4.4.1] shortcut ${label}: ` +
      `${accelerator}, registered=${registered}`
    );
  } catch (error) {
    console.error(
      `[Integration v4.4.1] shortcut ${label} failed:`,
      error.message
    );
  }
}
...
ipcMain.on(
  ...
  (event, state) => {
    ...
    if (
      ...
      console.log(
        "[Integration v4.4.1] dialog shape locked:",
        lockedDialogRect
      );
    }
...
app.whenReady().then(() => {
  ...
  console.log(
    "[Integration v4.4.1] Electron:",
    process.versions.electron
  );

  console.log(
    "[Integration v4.4.1] userData:",
